...
void DisplayMenu::show()
{
    ...
    for (int i = 0; i < products.size() / MAX_LINES + 1; i++)
    {
        ...
        if (i == products.size() / MAX_LINES)
            builder << "Page " << i + 1 << "/" << products.size() / MAX_LINES + 1 << ". Displays " <<
            products.size() << " of " << products.size() << " products";
        else
        builder << "Page " << i + 1 << "/" << products.size() / MAX_LINES + 1 << ". Displays " 
            << (i + 1) * MAX_LINES << " of " << products.size() << " products";
        lines[i].push_back(builder.str());
        builder.str(std::string());
        builder << " ID|" << setw(30) << "Name|" << setw(10) << "Price|";
        lines[i].push_back(builder.str());
        builder.str(std::string());
        builder << "---|" << setw(30) << "--------------------|" << setw(10) << "---------|";
        lines[i].push_back(builder.str());
        ...
        lines[i].push_back("Use arrow keys to go up or down");
        lines[i].push_back("Enter to edit product, 'd' to delete product");
        lines[i].push_back("Press esc to go back to main menu");
    }
